...
{
  int c = show_image(buff_[(buffIndex_ + 1)%3], "YOLO V3x", waitKeyDelay_);
  if (c != -1) c = c%256;
  if (c == 27) {
      demoDone_ = 1;
      return 0;
...
void YoloObjectDetector::yolo()
{
  ...
  if (!demoPrefix_ && viewImage_) {
      cv::namedWindow("YOLO V311", cv::WINDOW_NORMAL);
    if (fullScreen_) {
      cv::setWindowProperty("YOLO V322", cv::WND_PROP_FULLSCREEN, cv::WINDOW_FULLSCREEN);
    } else {
      cv::moveWindow("YOLO V333", 0, 0);
      cv::resizeWindow("YOLO V344", 640, 480);
    }
  }
